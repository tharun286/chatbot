...
def _extract_last_history_message(history, role=None):
    """Return the latest history message matching role, if any."""
    if not isinstance(history, list):
        return None
    for msg in reversed(history):
        if isinstance(msg, dict) and (role is None or msg.get("role") == role):
            return msg
    return None


async def _get_pending_ticket(session_uuid, db):
    """Return pending ticket data saved in the current chat session."""
    if not session_uuid or not db:
        return None
    try:
        stmt = select(HLSChatbotHistory).where(
        ...
        history = json.loads(existing.chat_history or "[]")
        for msg in reversed(history):
            if isinstance(msg, dict) and "ticket_pending" in msg:
                if msg.get("ticket_pending") is True:
                    return {
                        "question": msg.get("ticket_question"),
                        "domain": msg.get("ticket_domain"),
                        "brand": msg.get("ticket_brand"),
                        "journey": msg.get("ticket_journey"),
                    }
                return None
    except Exception:
        pass
...
async def _save_ticket_state(
    ...
    *,
    ...
    """Persist the pending-ticket state in the existing chatbot history."""
    if not session_uuid or not db:
        return
...
def _classify_ticket_confirmation_llm(query, conversation_history, llm):
    """Classify a reply to a previously offered support ticket."""
    conv_str = str(conversation_history)[-3500:]
    system_text = (
        "You are a support-ticket confirmation classifier. "
        "The chatbot has already asked the user whether they want a support ticket. "
        "Classify ONLY the user's latest reply.\n\n"
        "Return ONLY valid JSON:\n"
        '{"confirmation": "yes" | "no" | "unclear"}\n\n'
        "YES means the user agrees to proceed or asks to create the ticket, "
        "including phrases such as yes, sure, okay, go ahead, create it, please create one, do it.\n"
        "NO means the user declines, including no, no thanks, don't create it, not now, cancel.\n"
        "UNCLEAR means the reply is neither a clear acceptance nor rejection.\n\n"
        "Conversation:\n{conv_str}\n\n"
        "Latest user reply:\n{query}"
    )
    ...
    try:
        ...
        if raw.startswith("```"):
            raw = raw.split("\n", 1)[-1].rsplit("```", 1)[0].strip()
        result = json.loads(raw)
        confirmation = result.get("confirmation", "unclear")
        if confirmation in ("yes", "no", "unclear"):
            return confirmation
    except Exception:
        pass
    return "unclear"
...
def _classify_ticket_need_llm(query, conversation_history, context, llm):
    """Decide whether the latest request can be handled by the chatbot or needs a human."""
    conv_str = str(conversation_history)[-3500:]
    context_str = str(context)[:8000]
    system_text = (
        "You are the support-escalation classifier for a chatbot.\n\n"
        "Determine whether the user's latest request can be answered accurately using "
        "the supplied knowledge context or requires a human support representative.\n\n"
        "Return ONLY valid JSON:\n"
        '{"resolution": "answerable" | "human_required", '
        '"confidence": 0.0, "reason": "short reason"}\n\n'
        "Rules:\n"
        "- answerable: the requested information is available in the context and the chatbot can provide it.\n"
        "- human_required: the user needs a person to investigate, access an account, take an action, "
        "handle an individual case, resolve an order/delivery/payment/refund/access problem, or otherwise "
        "do something the knowledge base cannot do.\n"
        "- Do NOT classify a request as human_required merely because the topic is difficult. "
        "If the context contains enough information to answer it, use answerable.\n"
        "- Do NOT create a ticket just because FAISS returned weak or unrelated context. Judge whether "
        "the actual request requires human action.\n\n"
        "Conversation:\n{conv_str}\n\n"
        "Latest user request:\n{query}\n\n"
        "Knowledge context:\n{context_str}"
    )
    ...
    try:
        ...
        print("TICKET CLASSIFIER RAW =", raw)
        if raw.startswith("```"):
            raw = raw.split("\n", 1)[-1].rsplit("```", 1)[0].strip()
        result = json.loads(raw)
        if result.get("resolution") in ("answerable", "human_required"):
            return result
    except Exception:
        pass

    # Fail safe: if the classifier fails, preserve the normal chatbot flow.
    print("Ticket classifier failed")
    return {
...
async def _create_support_ticket(
    ...
    """Create and persist a minimal support ticket."""
...
try:
        ...
        if any(x in query_lower for x in [
            ...
            "open ticket"]):
            return {
                "response": "I can create a support ticket for you. Would you like me to proceed?",
                "ticket_offer": True,
        ...
        action = data.get("action") or data.get("ticket_action")
        if not query and action != "create_ticket":
            return {
        ...
        # -------------------------------------------------------------------
        # Pending support-ticket flow
        # 1) A human-required query first asks a question only.
        # 2) If the user agrees, show the frontend Create Ticket button.
        # 3) The ticket is NOT created until the button sends action=create_ticket.
        # -------------------------------------------------------------------
        session_uuid = data.get("session_uuid")

        print("SESSION_UUID =", session_uuid)

        pending_ticket = await _get_pending_ticket(session_uuid, db)

        print("PENDING_TICKET =", pending_ticket)
      ...
      if action == "create_ticket":
        try:
            ...
            ticket = await _create_support_ticket(
                session_uuid=session_uuid,
                db=db,
                question=pending_ticket.get("question") or "",
                domain=pending_ticket.get("domain") or domain,
                brand=pending_ticket.get("brand"),
                journey=pending_ticket.get("journey"),
            ...
            ticket_question = pending_ticket.get("question") or ""
            ...
            return {
                ...
                "issue": ticket_question,
                "created_at": ticket.created_at.isoformat(),
                "show_view_ticket_button": True,
                "ticket_offer": False,
                "show_create_ticket_button": False,
                "session_uuid": session_uuid,
                "sources": [],
                "follow_up_questions": [],
            }
            ...
            await db.rollback()
            return {
                "error": f"Failed creating support ticket: {str(e)}"
            }

            # The user is responding to the ticket question. Do not create yet.
            confirmation = _classify_ticket_confirmation_llm(
                query, conversation_history, llm
            )

            if confirmation == "yes":
                return {
                    "response": "Sure. Please click below to create the support ticket.",
            ...
            if confirmation == "no":
                ...
                return {
                    "response": "No problem. I won't create a support ticket.",
                    "ticket_created": False,
                    "ticket_id": None,
                    "ticket_offer": False,
            ...
            return {
                "response": "Would you like me to create a support ticket for this issue?",
                "ticket_created": False,
                "ticket_id": None,
                "ticket_offer": True,
                "ticket_confirmation": False,
        ...
        # -------------------------------------------------------------------
        # Support escalation decision. This is separate from adverse-event
        # handling: the existing AE workflow remains unchanged.
        # -------------------------------------------------------------------
        query_lower = query.lower()
        if any(keyword in query_lower for keyword in SUPPORT_KEYWORDS):
            ...
            ticket_decision = _classify_ticket_need_llm(
            query=query,
            conversation_history=conversation_history,
            context=context,
            llm=llm,)
        print("TICKET_DECISION =", ticket_decision)
        requires_human = (
            ticket_decision.get("resolution") == "human_required"
            and not is_adverse_event)

        # For a human-required request, ask the question first. Do NOT show
        # the Create Ticket button until the user explicitly agrees.
        ticket_offer = False
        show_create_ticket_button = False
        ticket_confirmation = False
        ticket_created = False
        ...
        if requires_human:
            ...
            if domain == "hls":
                active_brand_name = active_brand.get("display_name") if 'active_brand' in locals() and active_brand else None

            ticket_answer = (
                "This issue requires support team assistance. "
                "Please click the Create Ticket button below and we'll raise a ticket for you."
            )

            # Store the exact original question so a later confirmation does not
            # lose the reason for which the ticket was requested.
            pending_session_uuid = session_uuid or str(_uuid.uuid4())
            session_uuid = pending_session_uuid
            ticket_offer = True
            ...
            try:
                ...
                new_turns = [
                    ...
                    },
                    {
                        ...
                        "timestamp": now.isoformat(),
                        "ticket_pending": True,
                        "ticket_question": query,
                        "ticket_domain": domain,
                        "ticket_brand": active_brand_name,
                        "ticket_journey": "hcp" if is_hcp else "patient",
                    },
                ]
                ...
                await db.rollback()
                return {
                    "error": f"Failed storing pending ticket request: {str(e)}"
                }

            return {
                
                "response": ticket_answer,
        ...
        return {
            ...
            "show_create_ticket_button": show_create_ticket_button,
