...
def _should_offer_ticket(conversation_history):

    history = str(conversation_history)

    question_count = history.count("?")
...
Rules:
Escalate ONLY after at least 2-3 troubleshooting questions.
IMPORTANT TROUBLESHOOTING LIMIT

Do not ask more than 5 troubleshooting questions.

Count the troubleshooting questions already asked in the conversation history.

If 5 or more troubleshooting questions have already been asked and the issue is still unresolved:

Respond exactly:

"This issue appears to require assistance from our support team. Would you like me to create a support ticket for you?"

Do not ask any additional troubleshooting questions.
...
2. Have you tried password reset?
3. Did you receive the reset email?
4. If reset email was not received,
   then offer ticket creation.

Do NOT offer ticket creation before enough troubleshooting information has been gathered.
1. Understand the user's issue.
2. First acknowledge the issue with empathy.
3. Ask ONLY ONE troubleshooting question.
4. Use conversation history.
5. Do NOT answer with:
   "I don't have that information".
6. Do NOT give a solution immediately.
7. Investigate first.
...
"I am Extremely Sorry for your inconvenience. This issue appears to require investigation by the support team. Would you like me to create a support ticket for you?"
...
def _needs_support_escalation(query: str) -> bool:
...
async def process_hls_chat(
    ...
    # Use the existing session when supplied; otherwise create one.
    if db is None:
        db = AsyncSessionLocal()
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Explicit ticket creation action
        # ------------------------------------------------------------------
        if action == "create_ticket":
            if not query:
                query = "Support request from chatbot"
        ...
        # ------------------------------------------------------------------
        # Initialize LLM / domain configuration
        # ------------------------------------------------------------------
        ...
        greeting = DOMAIN_GREETINGS.get(
            domain,
            DOMAIN_GREETINGS["hls"],
        )

        normalized_query = re.sub(r"\s+", " ", query.strip().lower())

        if normalized_query in greeting_phrases:
            return {
                "response": greeting,
        ...
        def _is_support_conversation(history):
            support_terms = [
                "order",
                "login",
                "password",
            ...
            return any(term in history_text for term in support_terms)

        query_lower = query.lower()

        is_support_query = any(
            keyword in query_lower
            for keyword in support_keywords
        )

        active_support_session = _is_support_conversation(
            conversation_history
        )

        # Knowledge / medical-information requests must go through RAG.
        # This is intentionally checked separately from support detection so
        # questions such as "What is Synapta?" or "Tell me about Verveen"
        # are answered from the knowledge base rather than by the support agent.
        knowledge_keywords = [
            "synapta",
            "verveen",
            "kinetix",
        ...
        # A knowledge request should go through RAG even if the conversation
        # history previously contained support-related terms.  For example,
        # after a login discussion, "What is Synapta?" should still use RAG.
        if is_knowledge_query:
            active_support_session = False
            is_support_query = False

        print("SUPPORT_QUERY =", is_support_query)
        print("KNOWLEDGE_QUERY =", is_knowledge_query)
        print("ACTIVE_SUPPORT_SESSION =", active_support_session)

        if is_support_query or active_support_session:
            if _should_offer_ticket(conversation_history):
                return {
                    "response": (
                        "I am sorry for the inconvenience. This issue appears "
                        "to require assistance from our support team. "
            ...
            if _needs_support_escalation(query):
                return {
                    "response": (
                        "This issue appears to require investigation by the "
                        "support team. Would you like me to create a support "
                        "ticket for you?"
                    ),
                    "ticket_offer": True,
                    "show_create_ticket_button": True,
        ...
        # Journey detection
        # ------------------------------------------------------------------
        stored_journey = await _get_stored_journey(
            session_uuid,
            db,
        )
        ...
        llm_journey = _classify_journey_llm(
            query,
            conversation_history,
            llm,
        )
        ...
        if domain == "hls" and not isinstance(context, dict):
            if isinstance(conversation_history, str) and conversation_history:
                ...
                prior_history = (
                    conversation_history[:last_newline]
                    if last_newline >= 0
                    else ""
                )
            else:
            ...
            if active_brand:
                ...
                brand_switched = (
                    detected_brand is not None
                    and previous_brand is not None
                    and detected_brand["display_name"]
                    != previous_brand["display_name"]
                )

                system_prompt = _build_hls_brand_prompt(
                    active_brand,
                    is_first_brand_message=is_first_brand_message,
        ...
        # IMPORTANT: the final RAG return stays OUTSIDE this try/except.
        # ------------------------------------------------------------------
        try:
            llm_json = json.loads(raw_response)

            answer = (
                llm_json.get("response")
                or llm_json.get("answer")
                or raw_response
            )

            llm_sources = llm_json.get("sources", [])
            follow_up_questions = llm_json.get(
                "follow_ups",
                [],
            )
            show_msl_button = llm_json.get(
                "show_msl_button",
                is_hcp,
            )
        ...
        # ------------------------------------------------------------------
        # Final RAG response
        # This MUST be outside the JSON parsing except block.
        # ------------------------------------------------------------------
        return {
            "response": answer,
        ...
        print("ERROR processing HLS chat:", exc)
